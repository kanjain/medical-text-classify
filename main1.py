...
def main(test_model_locally):
    ...
    file_train_stemmed = 'dataset/train/train_stemmed.dat'
    file_train_labels = 'dataset/train/train_labels.dat'
    file_test_stemmed = 'dataset/test/test_stemmed.dat'
    ...
    # load train data
    if os.path.isfile(file_train_stemmed) == False:
        ...
        train_file = open(file_train_stemmed, 'w')
        for item in dataList:
            train_file.write("%s\n" % item)
        train_label_file = open(file_train_labels, 'w')
        for item in categoryList:
            train_label_file.write("%s\n" % item)
    else:
        with open(file_train_stemmed) as f:
            for line in f:
                dataList.append(line)
        with open(file_train_labels) as f:
            for line in f:
                categoryList.append(line)

    # load test data
    if os.path.isfile(file_test_stemmed) == False:
        with open(file_test) as f:
            reader = csv.reader(f, delimiter='\t')
            for row in reader:
                testDataList.append(normalize(row[0], stop_words))
        test_file = open(file_test_stemmed, 'w')
        for item in testDataList:
            test_file.write("%s\n" % item)
    else:
        with open(file_test_stemmed) as f:
            for line in f:
